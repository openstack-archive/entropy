...
def _get_script_metadata(backend, script_type):
...
# TODO(praneshp) last function to fix for backend
def _add_to_list(engine, script_type, script_name, **script_args):
    backend = _get_backend_from_engine(engine)
    script_metadata = _get_script_metadata(backend, script_type)
    if backend.check_duplicate(script_type, script_name):
